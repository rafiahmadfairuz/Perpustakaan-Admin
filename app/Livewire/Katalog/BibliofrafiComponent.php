...
class BibliofrafiComponent extends Component
{
    ...
    {
        try {
            $penerbitBaru = Penerbit::create([
                'nama_penerbit' => $this->pengarang['nama'],
                'tempat_id' => $this->tempat_id,
            ]);

            $this->refreshPenerbit();
            $this->penerbit_id = $penerbitBaru->id;
            $this->pengarang['nama'] = '';

            session()->flash('success', 'Penerbit baru berhasil ditambahkan.');
        } catch (\Exception $e) {
            session()->flash('error', 'Terjadi kesalahan: ' . $e->getMessage());
        }
    }
    ...
    {
        try {
            $tempatBaru = TempatPenerbit::create([
                'nama_tempat' => $this->nama_tempat,
            ]);

            $this->refreshTempat();
            $this->tempat_id = $tempatBaru->id;
            $this->nama_tempat = '';
